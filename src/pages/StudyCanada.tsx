...
import CompatibilityScoreSection from '../components/CompatibilityScoreSection';
import TrainingsCertificationsSection from '../components/TrainingsCertificationsSection';
import StudentSpaceChapter from '../components/StudentSpaceChapter';
import StudentSpaceQuickNav from '../components/StudentSpaceQuickNav';
import { CompatibilityProfileProvider } from '../context/CompatibilityProfileContext';
...
const StudyCanada = () => {
  ...
  useEffect(() => {
    const hasPrefill = (() => {
      try {
        return Boolean(sessionStorage.getItem(ORIENTATION_ADVISOR_PREFILL_KEY)?.trim());
      } catch {
        return false;
      }
    })();

    if (!hasPrefill && window.location.hash) {
    ...
    <div className="study-canada-page">
      <header className="student-space-header">
        <div className="container">
          <div className="student-space-header-content">
            <div className="header-badge">
              <Sparkles size={16} aria-hidden />
              <span>{isEn ? 'Student Space' : 'Espace Étudiant'}</span>
            </div>
            <h1>{isEn ? 'Your study journey, step by step' : 'Votre parcours d\'études, étape par étape'}</h1>
            <p className="header-subtitle">
              {isEn
                ? 'Explore schools in Senegal, compare costs, get AI guidance, and prepare your study abroad file for Canada — all in one place.'
                : "Explorez les écoles au Sénégal, comparez les coûts, bénéficiez du conseiller IA et préparez votre dossier d'études au Canada — tout au même endroit."}
            </p>
            <div className="student-space-header-actions">
              <button
                type="button"
                className="filter-btn active"
                onClick={() => scrollToSection('compat-score-title')}
              >
                <GraduationCap size={18} aria-hidden />
                {isEn ? 'Start exploring' : "Commencer l'exploration"}
              </button>
              <button
                type="button"
                className="filter-btn"
                onClick={() => scrollToSection('study-procedures-title')}
              >
                <FileText size={18} aria-hidden />
                {isEn ? 'Canada registration' : 'Inscription Canada'}
              </button>
            </div>
          </div>
        </div>
      ...
      <StudentSpaceQuickNav />

      <main className="study-canada-main">
        <CompatibilityProfileProvider>
          <StudentSpaceChapter
            id="etudiant-orienter"
            step={1}
            title={isEn ? 'Explore & get oriented' : "S'orienter et explorer"}
            subtitle={
              isEn
                ? 'Evaluate your profile, find schools, discover in-demand training and chat with our AI advisor.'
                : 'Évaluez votre profil, trouvez une école, découvrez les formations tendance et échangez avec notre conseiller IA.'
            }
            variant="accent"
          >
            <CompatibilityScoreSection />
            <SchoolFinderSection />
            <TrainingsCertificationsSection />
            <OrientationAdvisorChat />
          </StudentSpaceChapter>

          <StudentSpaceChapter
            id="etudiant-planifier"
            step={2}
            title={isEn ? 'Compare & plan your budget' : 'Comparer et planifier son budget'}
            subtitle={
              isEn
                ? 'Review tuition fees across institutions and estimate your annual student budget.'
                : 'Comparez les frais de scolarité entre établissements et estimez votre budget annuel d\'étudiant.'
            }
            variant="muted"
          >
        ...
        <StudentSpaceChapter
          id="etudiant-canada"
          step={3}
          title={isEn ? 'Study in Canada — registration' : 'Étudier au Canada — inscription'}
          subtitle={
            isEn
              ? 'Open your file, review required documents, fees and submit your information form.'
              : 'Ouvrez votre dossier, consultez les pièces à fournir, les frais et envoyez votre fiche de renseignement.'
          }
          variant="default"
        >
          <div className="study-canada-registration">
            <div className="container study-canada-registration-header">
              <span className="section-badge study-canada-registration-badge">
                <FileText size={14} aria-hidden />
                {isEn ? 'Registration' : 'Inscription'}
              </span>
              <h2 id="study-procedures-title" className="study-canada-registration-title">
                {isEn ? 'Procedures & information form' : 'Procédures & fiche de renseignement'}
              </h2>
            </div>
              ...
              <div className="study-procedure-column">
                <article className="study-card study-procedure-card">
                  <div className="study-card-header">
                    ...
                    <h3>{isEn ? 'Our mission' : 'Notre mission'}</h3>
                  </div>
                  <p>
                    {isEn
                ...
                <aside className="study-contact-card">
                  ...
                  <ul className="study-contact-list">
                    ...
                    <li>
                      ...
                      <span>2 voies liberté 6 en face camp pénal, lot 3 D3</span>
                    </li>
                  </ul>
                  <p className="study-contact-tagline">
                    {isEn ? 'Your success matters to us' : 'Nous avons votre succès à cœur'}
                  </p>
                </aside>
              </div>

              <article className="study-card study-fiche-card" id="study-fiche-form">
                <div className="study-card-header">
                  <FileText size={24} />
                  <h3>{isEn ? 'Information form' : 'Fiche de renseignement'}</h3>
                </div>
                <p>
                  {isEn
                ...
                >
                  ...
                  <div className="study-step-panel" data-step-panel="3" hidden={step !== 3}>
                    ...
                    <fieldset className="study-fieldset">
                      <legend>
                        {isEn ? 'Schools attended — University (if applicable)' : 'Établissements fréquentés — Université (si applicable)'}
                      </legend>
                      <label className="study-field">
                      ...
                      <button type="submit" className="btn btn-primary study-submit-btn" disabled={submitting}>
                        ...
                        {submitting
                          ? isEn
                            ...
                            : 'Envoi en cours…'
                          : isEn
                            ? 'Submit my information form'
                            : 'Envoyer ma fiche de renseignement'}
                      </button>
                    )}
                  </div>
                </form>
